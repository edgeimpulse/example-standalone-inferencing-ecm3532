...
//
// Maintain the current power state of the DSP Complex
//
static tDspPowerState    sregDspPowerState = eDspPowerOff;
...
EtaCspDspPowerUp(void)
{
    ...
    //
    // Indicate the DSP Complex is powered up at this point
    //
    sregDspPowerState = eDspPowerOn;
...
EtaCspDspPowerDown(void)
{
    ...
    //
    // Indicate the DSP Complex is powered off at this opint
    //
    sregDspPowerState = eDspPowerOff;
...
/***************************************************************************//**
 *
 *  EtaCspDspPowerStateGet - Return the power state (off/on) of
 *  the DSP Complex
 *
 *  @return Return the power state of the DSP Complex
 *
 ******************************************************************************/
tDspPowerState
EtaCspDspPowerStateGet(void)
{
    return (sregDspPowerState);
}


/***************************************************************************//**
 *
 *  EtaCspDspResetAssert - Assert the reset to the DSP Core
 *  (only)
 *
 *
 ******************************************************************************/
void
EtaCspDspResetAssert(void)
{
    REG_DSPCTRL_M3_DSP_CTRL.BF.RESET_DSP = 1;
}


/***************************************************************************//**
 *
 *  EtaCspDspResetDeassert - Remove the reset from the DSP Core
 *  (only)
 *
 *
 ******************************************************************************/
void
EtaCspDspResetDeassert(void)
{
                    ...
                    const tDspMem *psDspMem)
{
    ...
    {
        DSPPRINT_DEBUG EtaCspIoPrintf("  Errors detected. Leaving DSP in reset.\r\n");
    }
    else
    {
        //
        ...
        //
        DSPPRINT_DEBUG EtaCspIoPrintf("Taking DSP out of reset\r\n");
        REG_DSPCTRL_M3_DSP_CTRL.BF.RESET_DSP = 0;
    }
...
EtaCspDspLoaderCheck(bool bQuiet, bool bDisplayErr, const tDspMem *psDspMem)
{
    ...
    for(ui32Index0 = 0; ui32Index0 < psDspMem->ui32PmemSize; ui32Index0++)
    {
        ...
        if(ui32ExpData != ui32ReadData)
        {
            ...
            if(bDisplayErr)
            {
                 DSPPRINT_DEBUG EtaCspIoPrintf("  ERROR: PMEM [0x%x] act:0x%x exp:0x%x\r\n",
    ...
    for(ui32Index0 = 0; ui32Index0 < psDspMem->ui32XmemSize; ui32Index0++)
    {
        ...
        if(ui32ExpData != ui32ReadData)
        {
            ...
            if(bDisplayErr)
            {
                DSPPRINT_DEBUG EtaCspIoPrintf("  ERROR: XMEM [0x%x] act:0x%x exp:0x%x\r\n",
                               ui32Index0, ui32ReadData, ui32ExpData);
            }
        }
        pui32Act++;
    ...
    for(ui32Index0 = 0; ui32Index0 < psDspMem->ui32YmemSize; ui32Index0++)
    {
        ...
        if(ui32ExpData != ui32ReadData)
        {
            ...
            if(bDisplayErr)
            {
                 DSPPRINT_DEBUG EtaCspIoPrintf("  ERROR: YMEM [0x%x] act:0x%x exp:0x%x\r\n",
                               ui32Index0,
...
void EtaCspDspSemaphoreClear(tSemaphoreType semaphoreType)
{
  ...
  switch (semaphoreType)
  {
    ...
    case (eSemaphoreReadRequest0) :
      REG_DSPCTRL_M3_SEMAPHORE0.BF.SEMAPHORE = 0x1;
      break;

    case (eSemaphoreReadRequest1) :
      REG_DSPCTRL_M3_SEMAPHORE1.BF.SEMAPHORE = 0x1;
      break;

    case (eSemaphoreReadRequest2) :
      REG_DSPCTRL_M3_SEMAPHORE2.BF.SEMAPHORE = 0x1;
      break;

    case (eSemaphoreReadRequest3) :
    default :
      REG_DSPCTRL_M3_SEMAPHORE3.BF.SEMAPHORE = 0x1;
      break;
  }
  return;
}
...
/***************************************************************************//**
 *
 *  EtaCspDspWaitForSemaphore - Poll forever until a specific semaphore is set (presumably by other processor)
 *
 ******************************************************************************/
 ...
 *
 * @param tSemaphoreType is one of the 12 available semaphore types.
 *
