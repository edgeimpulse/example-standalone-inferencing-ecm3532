...
BUILD_DIR=$SCRIPTPATH/Applications/edge-impulse-standalone/build
cd $BUILD_DIR

# Generate a partition table file
python3 $SCRIPTPATH/Tools/bootloader/genPartitionTable.py $BUILD_DIR/edge-impulse-standalone.map
...
arm-none-eabi-objcopy -O binary --only-section=.dspBlock edge-impulse-standalone.elf 03_dsp_fw.bin 2> /dev/null
arm-none-eabi-objcopy -O binary --only-section=.pbufBlock edge-impulse-standalone.elf 04_pbuf.bin 2> /dev/null
arm-none-eabi-objcopy -O binary --remove-section=.pbufBlock --remove-section=.dspBlock edge-impulse-standalone.elf 02_m3_fw.bin 2> /dev/null
...
STTY=$(which stty || true)
if [ ! -x "$STTY" ]; then
    echo "Cannot find 'stty' in your PATH. Install stty before you continue."
    echo "For example, via: `sudo apt install -y coreutils`"
    exit 1
fi

SB=$(which sb || true)
if [ ! -x "$SB" ]; then
    echo "Cannot find 'sb' in your PATH. Install sb before you continue."
    echo "For example, via: `sudo apt install -y lrzsz`"
    exit 1
fi
...
python3 $SCRIPTPATH/bl_download.py $BUILD_DIR 0
