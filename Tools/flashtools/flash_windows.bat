...
set "CURR_DIR=%cd%"
set BUILD_DIR=..\..\Applications\edge-impulse-standalone\build
python -u ..\bootloader\genPartitionTable.py %BUILD_DIR%\edge-impulse-standalone.map
...
arm-none-eabi-objcopy -O binary --only-section=.dspBlock edge-impulse-standalone.elf 03_dsp_fw.bin
arm-none-eabi-objcopy -O binary --only-section=.pbufBlock edge-impulse-standalone.elf 04_pbuf.bin
arm-none-eabi-objcopy -O binary --remove-section=.pbufBlock --remove-section=.dspBlock edge-impulse-standalone.elf 02_m3_fw.bin
