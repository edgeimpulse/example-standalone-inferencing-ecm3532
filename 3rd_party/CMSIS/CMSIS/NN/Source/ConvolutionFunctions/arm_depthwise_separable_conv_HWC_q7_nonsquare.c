 ...
 */
    ...
    int       i_out_y, i_out_x, i_ch_in, i_ch_out;
    int       i_ker_y, i_ker_x;
    int       in_row, in_col;
    int       conv_out;
    ...
    for (i_ch_out = 0; i_ch_out < ch_im_out; i_ch_out++)
    {
        for (i_out_y = 0; i_out_y < dim_im_out_y; i_out_y++)
        {
            for (i_out_x = 0; i_out_x < dim_im_out_x; i_out_x++)
            {
                ...
                {
                    // for each output 
                    conv_out = ((q31_t)(bias[i_ch_in]) << bias_shift) + NN_ROUND(out_shift);
                    //conv_out = 0;
                    for (i_ker_y = 0; i_ker_y < dim_kernel_y; i_ker_y++)
                    {
                        for (i_ker_x = 0; i_ker_x < dim_kernel_x; i_ker_x++)
                        {
                            ...
                            {
                                conv_out += Im_in[(in_row * dim_im_in_x + in_col) * ch_im_in + i_ch_in] *                        
                                    wt[i_ch_out*dim_kernel_y*dim_kernel_x*ch_im_in + (i_ker_y*dim_kernel_x+i_ker_x)*ch_im_in + i_ch_in];
                            }
                        }
                    }
                    Im_out[(i_out_y * dim_im_out_x + i_out_x) * ch_im_in * ch_im_out + i_ch_in * ch_im_out + i_ch_out] =
